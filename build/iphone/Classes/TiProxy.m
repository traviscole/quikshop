...
//Rare exceptions to indicate a bug in the _quikshop code (Eg, method that a subclass should have implemented)
...
+ (id)createProxy:(NSString*)qualifiedName withProperties:(NSDictionary*)properties inContext:(id<TiEvaluator>)context
{
	...
	if (proxyClass == nil) {
		NSString *_quikshop = [NSString stringWithFormat:@"%@%s",@"Ti","tanium."];
		if ([qualifiedName hasPrefix:_quikshop]) {
		...
		if (proxyClass==nil) {
			...
			@throw [NSException exceptionWithName:@"org.quikshop.module"
